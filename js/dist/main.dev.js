...
/** @format */
var alignHorizontal = document.querySelector('.alignment__icon--horizontal');
var alignVertical = document.querySelector('.alignment__icon--vertical');
var cardsWrapper = document.querySelector('.cards__wrapper');
...
function navLuncher(e) {
  if (e.target.classList.contains('nav__menu') || e.target.classList.contains('nav__menu--icon') || e.target.classList.contains('nav__menu--path')) {
...
function allGames(games) {
  ...
  games.forEach(function (game, index) {
    var id = game.id,
        ...
        rating = game.rating,
        genres = game.genres;
    newGame += "\n\t\t<div class=\"card\" id=".concat(id, ">\n\t\t<div class=\"card__img card__img--format\" style=\"background-image: url(").concat(background_image || '../img/placeholder.jpg', ");\"></div>\n\t\t<div class=\"card__info card__info--format\">\n\t\t\t<h3 class=\"header bold open-modal\">").concat(name, "</h3>\n\t\t\t<div class=\"consoles\">\n\t\t\t<ul>");

    for (var i = 0; i < parent_platforms.length; i++) {
      parent_platforms[i].platform.slug; // newGame += `<li>${platform}</li>`;

      if (parent_platforms[i].platform.slug === 'playstation') {
        newGame += "<li><div class=\"console__icon\">".concat(playstationIcon, "</div></li>");
      }

      if (parent_platforms[i].platform.slug === 'xbox360') {
        newGame += "<li><div class=\"console__icon\">".concat(xboxIcon, "</div></li>");
      }

      if (parent_platforms[i].platform.slug === 'pc') {
        newGame += "<li><div class=\"console__icon\">".concat(pcIcon, "</div></li>");
      }

      if (parent_platforms[i].platform.slug === 'nintendo') {
        newGame += "<li><div class=\"console__icon\">".concat(nintendoIcon, "</div></li>");
      }

      if (parent_platforms[i].platform.slug === 'ios') {
        newGame += "<li><div class=\"console__icon\">".concat(iosIcon, "</div></li>");
      }

      if (parent_platforms[i].platform.slug === 'mac') {
        newGame += "<li><div class=\"console__icon\">".concat(macIcon, "</div></li>");
      }

      if (parent_platforms[i].platform.slug === 'linux') {
        newGame += "<li><div class=\"console__icon\">".concat(linuxIcon, "</div></li>");
      }

      if (parent_platforms[i].platform.slug === 'android') {
        newGame += "<li><div class=\"console__icon\">".concat(androidIcon, "</div></li>");
      }
    }

    newGame += "</ul>\n\t\t\t</div>\n\t\t\t<div class=\"release--card\">\n\t\t\t\t<p>Release Date</p>\n\t\t\t\t<p>".concat(formatDate(released), "</p>\n\t\t\t</div>\n\t\t\t<p class=\"rank bold\">#").concat(index + 1, "</p>\n\t\t\t<div class=\"genre--card\">\n\t\t\t\t<p>Genres</p><p>");

    for (var _i = 0; _i < genres.length; _i++) {
      var genre = genres[_i];
      newGame += "".concat(genre.name);

      if (_i < genres.length - 1) {
        newGame += ", ";
      }
    }
...
function closeModalFn(e) {
  if (e.target.classList.contains('modal') || e.target.classList.contains('modal__close') || e.target.classList.contains('modal__close--path')) {
...
function escKeyboard(e) {
  if ((e.keyCode = 'escape') && !document.querySelector('#modal').classList.contains('modal--hidden')) {
    document.querySelector('#modal').classList.add('modal--hidden');
    document.body.style.overflowY = 'scroll';
    enableScroll();
  }
...
document.querySelectorAll('.card__info--format'); // Change card image on vertical/horizontal display

document.querySelectorAll('.card__img--format'); // Horizontal display
...
function alignHorizontalFn(e) {
  ...
  document.querySelector('.card__info--format').classList.replace('card__info--vertical', 'card__info');
  document.querySelector('.card__img--format').classList.replace('card__img--vertical', 'card__img');
  var cardImg = document.querySelectorAll('.card__img--format');

  for (var i = 0; i < cardImg.length; i++) {
    var eachCardImage = cardImg[i];
    eachCardImage.classList.replace('card__img--vertical', 'card__img');
  }

  var cardInfo = document.querySelectorAll('.card__info--format');

  for (var _i2 = 0; _i2 < cardInfo.length; _i2++) {
    var eachCardInfo = cardInfo[_i2];
    eachCardInfo.classList.replace('card__info--vertical', 'card__info');
  }

  var cardDescription = document.querySelectorAll('.card__description--format');

  for (var _i3 = 0; _i3 < cardDescription.length; _i3++) {
    var eachcardDescription = cardDescription[_i3];
...
function alignVerticalFn() {
  ...
  for (var _i4 = 0; _i4 < cardInfo.length; _i4++) {
    var eachCardInfo = cardInfo[_i4];
    eachCardInfo.classList.replace('card__info', 'card__info--vertical');
  }

  var cardDescription = document.querySelectorAll('.card__description--format');

  for (var _i5 = 0; _i5 < cardDescription.length; _i5++) {
    var eachcardDescription = cardDescription[_i5];
...
function makeModal(id) {
  ...
  var _modalInformation = modalInformation,
      background_image = _modalInformation.background_image,
      parent_platforms = _modalInformation.parent_platforms,
      platforms = _modalInformation.platforms,
      platform = _modalInformation.platforms.platform,
      genres = _modalInformation.genres,
      developers = _modalInformation.developers,
      released = _modalInformation.released,
      description = _modalInformation.description,
      publishers = _modalInformation.publishers,
      esrb_rating = _modalInformation.esrb_rating,
      website = _modalInformation.website,
  ...
  for (var _i6 = 0; _i6 < parent_platforms.length; _i6++) {
    ...
    if (parent_platforms[_i6].platform.slug === 'playstation') {
      newModal += "<li><div class=\"modal__consoles--icon\">".concat(playstationIcon, "</div></li>");
    }

    if (parent_platforms[_i6].platform.slug === 'xbox360') {
      newModal += "<li><div class=\"modal__consoles--icon\">".concat(xboxIcon, "</div></li>");
    }

    if (parent_platforms[_i6].platform.slug === 'pc') {
      newModal += "<li><div class=\"modal__consoles--icon\">".concat(pcIcon, "</div></li>");
    }

    if (parent_platforms[_i6].platform.slug === 'nintendo') {
      newModal += "<li><div class=\"modal__consoles--icon\">".concat(nintendoIcon, "</div></li>");
    }

    if (parent_platforms[_i6].platform.slug === 'ios') {
      newModal += "<li><div class=\"modal__consoles--icon\">".concat(iosIcon, "</div></li>");
    }

    if (parent_platforms[_i6].platform.slug === 'mac') {
      newModal += "<li><div class=\"modal__consoles--icon\">".concat(macIcon, "</div></li>");
    }

    if (parent_platforms[_i6].platform.slug === 'linux') {
      newModal += "<li><div class=\"modal__consoles--icon\">".concat(linuxIcon, "</div></li>");
    }

    if (parent_platforms[_i6].platform.slug === 'android') {
      newModal += "<li><div class=\"modal__consoles--icon\">".concat(androidIcon, "</div></li>");
    }
  }
  ...
  for (var _i7 = 0; _i7 < platforms.length; _i7++) {
    ...
    newModal += "".concat(_platform.name);

    if (_i7 < platforms.length - 1) {
      newModal += ", ";
    }
  }
  ...
  for (var _i8 = 0; _i8 < genres.length; _i8++) {
    var genre = genres[_i8];
    newModal += "".concat(genre.name);

    if (_i8 < genres.length - 1) {
      newModal += ", ";
    }
  }

  newModal += "</p>\n\t\t\t</div>\n\t\t\t<div class=\"release\">\n\t\t\t\t<p class=\"spec__title\">Release Date</p>\n\t\t\t\t<p class=\"spec__desc\">".concat(formatDate(released), "</p>\n\t\t\t</div>\n\t\t\t<div class=\"developer\">\n\t\t\t\t<p class=\"spec__title\">Developer</p>\n\t\t\t\t<p class=\"spec__desc underline\">");

  for (var _i9 = 0; _i9 < developers.length; _i9++) {
    var developer = developers[_i9];
    newModal += "".concat(developer.name);

    if (_i9 < developers.length - 1) {
      newModal += ", ";
    }
  }
  ...
  for (var _i10 = 0; _i10 < publishers.length; _i10++) {
    var publisher = publishers[_i10];
    newModal += "".concat(publisher.name);
